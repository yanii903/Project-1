...
switch ($client) {
    ...
    case 'pay':
        ...
        if (isset($_POST['btnPay'])) {
            ...
            if (empty($phonePay)) {
                $messPhonePay = "SĐT không được để trống!";/-strong/-heart:>:o:-((:-h $check_valid_order = false;
            } elseif (!preg_match('/^[0-9]{10,11}$/', $phonePay)) {
                $messPhonePay = "SĐT không hợp lệ!";
                $check_valid_order = false;
            }
        ...
        // Hiển thị giao diện form thanh toán
        include 'pay.php';
        break;​12:17/-strong/-heart:>:o:-((:-h Xem trước khi gửiThả Files vào đây để xem lại trước khi gửi
    case 'profile':
        $list_account = load_all_account(); // Lấy dữ liệu từ CSDL
