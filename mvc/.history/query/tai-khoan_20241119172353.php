...
{
    $sql = "INSERT INTO `account`(`tk_user`, `tk_password`, `tk_email`, `tk_address`, `id_role`) 
            VALUES (?, ?, ?, ?, ?)";
    ...
    $tk_id = pdo_last_insert_id();

    // Kiểm tra id trả về và thực hiện thêm giỏ hàng
    if ($tk_id > 0) {
        insert_cart($tk_id);
    } else {
        throw new Exception("Failed to get user ID after account creation.");
    }
}
