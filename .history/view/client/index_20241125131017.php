...
switch ($client) {
    ...
    case 'pay':
        ...
        // Lấy thông tin từng sản phẩm từ database và kiểm tra số lượng tương ứng
        foreach ($spids as $index => $spid) {
            $product = load_one_product(intval($spid)); // Hàm truy vấn sản phẩm theo ID
            $quantity = isset($quantities[$index]) ? intval($quantities[$index]) : 1; // Lấy số lượng sản phẩm (mặc định là 1 nếu không có)
            if ($product) {
        ...
        if (isset($_POST['btnPay'])) {
            ...
            if ($check_valid_order) {
                ...
                foreach ($list_products as $product) {
                    // Lấy thông tin từng sản phẩm từ mảng $product
                    $sp_price = $product['sp_price']; // Giá sản phẩm
                    // $quantity = isset($product['quantity']) ? $product['quantity'] : 1; // Số lượng sản phẩm, mặc định là 1 nếu không có
                    $quantity = $_GET['totalquantity'];

                    // Lấy ID sản phẩm từ URL
                    $sp_id = $product['sp_id']; // ID sản phẩm trong mảng $product
                    $sp_quantity = $product['sp_id']; // ID sản phẩm trong mảng $product
