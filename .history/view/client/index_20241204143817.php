...
switch ($client) {
    ...
    case 'pay':
        ...
        $spid_counts = array_count_values($spids); // Đếm số lần xuất hiện của mỗi sản phẩm

        foreach ($spid_counts as $spid => $count) {
            $product = load_one_product(intval($spid)); // Hàm truy vấn sản phẩm theo ID

            if ($product) {
                $product['quantity'] = $count; // Thêm thông tin số lượng vào sản phẩm
                $list_products[] = $product; // Thêm sản phẩm vào danh sách
                $totalAmount += $product['sp_price'] * $count; // Cộng dồn tổng tiền (giá * số lượng)
            }
        }
