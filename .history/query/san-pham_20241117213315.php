...
function insert_product($sp_name, $sp_image, $sp_price, $sp_quantity, $sp_describe, $id_dm)
{
    $sql = "INSERT INTO `product`(`sp_name`, `sp_image`, `sp_price`, `sp_title`, `sp_quantity`, `sp_describe`, `id_dm`)
    VALUES
...
{
    $sql = "UPDATE `product` SET  `sp_name` = '" . $sp_name . "', `sp_image` = '" . $sp_image . "', `sp_price` = '" . $sp_price . "', `sp_title` = '" . $sp_title . "', `sp_quantity` = '" . $sp_quantity . "',  `sp_describe` = '" . $sp_describe . "',  `id_dm` = '" . $id_dm . "'WHERE `sp_id` = '" . $sp_id . "'";
    pdo_execute($sql);
} //cập nhật sản phẩm
